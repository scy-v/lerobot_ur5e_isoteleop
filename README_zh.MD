...
### 2.4 获取主臂-从臂的零点偏差
> **⚠️ 注意：cfg.yaml中的`hardware_offsets`应保存你的主从臂各关节之间的零点偏差角度，以确保主从臂零点关节姿态完全一致。**
每次执行此操作前，请单独拖动主臂，使其测试关节角与从臂测试关节角尽量保持一致，然后运行以下命令并读取 `HardWare_offsets` 中对应的角度值填入`cfg.yaml`中:
```bash
utils-joint-offsets
```
### 2.5 获取主臂-从臂关节角的误差周期
