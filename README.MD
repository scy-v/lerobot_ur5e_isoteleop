...
- Before each operation, align the master arm's test joint angles with the slave arm's, then run the command and read the angles from `HardWare_offsets` into `cfg.yaml`:
```bash
utils-joint-offsets
```
